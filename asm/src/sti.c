...
/*   Updated: 2017/08/25 14:34:27 by hstander         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			create_acb(int fd, char *arg1, char *arg2, char *arg3)
{
	...
	if (arg1[0] != 'r')
	{
		ft_printf("Invalid parameter 1 for sti, should be a register\n");
		exit (0);
	}	
	hex =  0b01000000;
	if (arg2[0] == '%')
	...
	{
		ft_printf("Invalid parameter 3 for sti, should be a T_DIR | T_REG\n");
		exit (0);
	}
	if (write(fd, (void *)&hex, 1) < 0)
	{
		ft_printf("Unable to write sti's argument coding byte to file - exiting\n");
		exit(1);
	}
}
